        ...
        if (task.status === 'Completed') {
            task.todoChecklist.forEach(item => {
                ...
                if (Array.isArray(item.subtasks)) {
                    item.subtasks.forEach(st => { st.completed = true; });
                }
            });
            task.progress = 100;
        }
...
const updateTaskChecklist = async (req, res) => {
    try {
        ...
        // Derive parent `completed` from subtasks: a main task is complete only
        // when ALL its subtasks are checked. Items without subtasks keep their own value.
        const normalized = (todoChecklist || []).map(item => {
        ...
        task.todoChecklist = normalized;

        // Auto-update progress based on parent checklist completion
        const completedCount = normalized.filter(item => item.completed).length;
        const totalItems = normalized.length;
        task.progress = totalItems > 0 ? Math.round((completedCount / totalItems) * 100) : 0;

        // Auto-update as completed if all items are checked
